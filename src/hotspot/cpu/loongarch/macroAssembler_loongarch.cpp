...
void MacroAssembler::wrap_label(Register r, Label &L, reg_addr_insn insn) {
  if (L.is_bound()) {
...
void MacroAssembler::call_VM_base(Register oop_result,
  ...
  if (is_preemptable(entry_point)) {
    ...
    set_last_Java_frame(last_java_sp, FP, l);
  }
...
void MacroAssembler::set_last_Java_frame(Register last_java_sp,
  ...
  lea_long(tmp, AddressLiteral(last_java_pc, relocInfo::relocType::none));
  st_d(tmp, Address(TREG, JavaThread::frame_anchor_offset() + JavaFrameAnchor::last_Java_pc_offset()));
...
void MacroAssembler::lea(Register dst, const address addr) {
  int64_t distance = addr - pc();
  int64_t offset = distance & 0b1111111111111111111100;
  pcaddi(dst, offset);
  if (offset != distance) {
    addi_d(dst, dst, distance - offset);
  }
}

void MacroAssembler::lea(Register dst, Label &addr) {
  wrap_label(dst, addr, &MacroAssembler::lea);
}
