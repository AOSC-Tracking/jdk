...
class StubGenerator: public StubCodeGenerator {
 private:
  ...
  address generate_cont_preempt_stub() {
    ...
    __ ld_bu(T0, Address(TREG, JavaThread::preemption_cancelled_offset()));
    ...
    __ lea(FP, Address(SP, checked_cast<int32_t>(ContinuationEntry::size() + 2 * wordSize)));
    __ lea_long(T1, ExternalAddress(ContinuationEntry::thaw_call_pc_address()));
    __ ld_d(T1, Address(T1));
    __ jr(T1);
